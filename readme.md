...
To make sure the dummy data worked, run `docker exec -it downtown-library ./cockroach sql --insecure --database=library_system` to run a SQL session.
Then, enter a simple SQL query, such as `SELECT book_name FROM books_A;`.
